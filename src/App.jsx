...
const AiIcon = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <path d="M17 14v4a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4" />
    <path d="M18 1 Q 18 5 22 5 Q 18 5 18 9 Q 18 5 14 5 Q 18 5 18 1 Z" fill="currentColor" stroke="none" />
    <path d="M22 9 Q 22 11 24 11 Q 22 11 22 13 Q 22 11 20 11 Q 22 11 22 9 Z" fill="currentColor" stroke="none" />
    <path d="M13 2 Q 13 4 15 4 Q 13 4 13 6 Q 13 4 11 4 Q 13 4 13 2 Z" fill="currentColor" stroke="none" />
    <path d="M8.5 16l1.5-6 1.5 6M9 13.5h2" />
    <path d="M14 10v6" />
  </svg>
);
                    ...
                    <tr key={r.id || idx}>
                      ...
                      <td>
                        <span style={{
                          ...
                          background: (r.remarks && r.remarks.includes('Disqualified')) || r.terminationReason === 'tab_switch' ? 'rgba(239,68,68,0.2)' : r.percentage >= 80 ? 'rgba(34,197,94,0.15)' : r.percentage >= 50 ? 'rgba(56,189,248,0.15)' : 'rgba(239,68,68,0.15)',
                          color: (r.remarks && r.remarks.includes('Disqualified')) || r.terminationReason === 'tab_switch' ? '#ef4444' : r.percentage >= 80 ? '#86efac' : r.percentage >= 50 ? '#38bdf8' : '#fca5a5',
                          fontWeight: 800, fontSize: 11.5
                        }}>
                          {(r.remarks && r.remarks.includes('Disqualified')) || r.terminationReason === 'tab_switch' ? 'Disqualified (Tab Switch)' : r.percentage >= 80 ? 'Distinction' : r.percentage >= 50 ? 'Passed' : 'Needs Review'}
                        </span>
                      </td>
              ...
              {showAiImport && (
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 16, marginBottom: 16 }}>
                  ...
                  <details style={{ marginBottom: 16, cursor: 'pointer' }}>
                    <summary style={{ fontSize: 12, color: '#38bdf8', userSelect: 'none', outline: 'none', marginBottom: 8, fontWeight: 600 }}>How does this work?</summary>
                    <div style={{ display: 'flex', gap: 16, marginTop: 8, cursor: 'default', flexWrap: 'wrap' }}>
                      <div style={{ flex: 1, minWidth: 200, fontSize: 12, color: 'rgba(255,255,255,0.5)', background: 'rgba(0,0,0,0.2)', padding: 12, borderRadius: 10, border: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 200, fontSize: 12, color: 'rgba(255,255,255,0.8)', background: 'rgba(56,189,248,0.08)', padding: 12, borderRadius: 10, border: '1px dashed rgba(56,189,248,0.3)' }}>
          ...
          <div style={{ textAlign: 'center', padding: '16px 8px' }}>
            {showReview ? (
              <div style={{ textAlign: 'left' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <h2 style={{ fontSize: 20, fontWeight: 800, color: '#fff', margin: 0 }}>Answer Review</h2>
                  <button onClick={() => setShowReview(false)} style={{ background: 'none', border: 'none', color: '#38bdf8', cursor: 'pointer', fontWeight: 600 }}>Back to Score</button>
                </div>
                <div style={{ maxHeight: '60vh', overflowY: 'auto', paddingRight: 8 }}>
                  {quiz.questions.map((q, i) => {
                    const studentAns = selectedAnswers[i];
                    const isCorrect = studentAns === q.correctIndex;
                    const skipped = studentAns === undefined;
                    return (
                      <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${isCorrect ? 'rgba(34,197,94,0.3)' : skipped ? 'rgba(255,255,255,0.1)' : 'rgba(239,68,68,0.3)'}`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
                        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 8 }}>Question {i + 1}</div>
                        <div style={{ fontSize: 15, color: '#fff', fontWeight: 600, marginBottom: 12, lineHeight: 1.4 }}>{q.question}</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {q.options.map((opt, optIdx) => {
                            ...
                            return (
                              <div key={optIdx} style={{ padding: '8px 12px', borderRadius: 8, background: bg, color, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
                                <div style={{ width: 20, height: 20, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700 }}>
              ...
              <>
                ...
                <div style={{
                  ...
                  <button
                    type="button"
                    className="btn-action btn-secondary"
                    style={{ maxWidth: 200, padding: '10px 16px' }}
              ...
              <div key={r.id || idx} className="res-row" style={{ flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                  ...
                  <span style={{
                    ...
                    background: (r.remarks && r.remarks.includes('Disqualified')) || r.terminationReason === 'tab_switch' ? 'rgba(239,68,68,0.2)' : r.percentage >= 80 ? 'rgba(34,197,94,0.15)' : 'rgba(56,189,248,0.15)',
                    color: (r.remarks && r.remarks.includes('Disqualified')) || r.terminationReason === 'tab_switch' ? '#ef4444' : r.percentage >= 80 ? '#86efac' : '#38bdf8',
